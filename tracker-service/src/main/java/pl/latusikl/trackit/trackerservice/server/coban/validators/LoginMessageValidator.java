 ...
 */
@Slf4j
class  LoginMessageValidator implements Predicate<String> {
	...
	public boolean test(final String s) {
		...
		if (splitMessage.length != LOGIN_PACKET_SIZE_AFTER_SPLIT) {
			logValidationError("split length", String.valueOf(splitMessage.length), String.valueOf(LOGIN_PACKET_SIZE_AFTER_SPLIT));
			return false;
		}
	...
	private void logValidationError(final String cause, final String actual, final String expected) {
		log.debug("Login message validation failed. Caused by: {} Actual: {} . Expected: {} .", cause, actual, expected);
	}
}
